...
class BuyFnbPage extends React.Component {
  ...
  getMenuOption() {
    ...
    axios
      ...
      .catch(error => {
        console.error('There was an error retrieving the food and beverage option:', error);
        console.log(menu_id);
        Swal.fire('Error', 'There was an error retrieving the food and beverage option', 'error');
      });
  }
      ...
      if (result.isConfirmed) {
        ...
        axios
          ...
          .catch(error => {
            ...
            Swal.fire('Error', 'There was an error purchasing the item', 'error');
          });
      }
    });
  }
      ...
      <>
        ...
        <div className="BuyFnbPage">
          <div className="BuyFnbPage--card">
            <img className="BuyFnbPage--image" src={menuOption.menuIMG} alt={menuOption.menu} />
            <div className="BuyFnbPage--details">
              <h2 className="BuyFnbPage--menu">{menuOption.menu}</h2>
              <p className="BuyFnbPage--description">{menuOption.menu_description}</p>
              <p className="BuyFnbPage--price">Price: ${menuOption.price.toFixed(2)}</p>
              <button className="BuyFnbPage--button" onClick={() => this.handleBuyNowClick()}>Buy Now</button>
            </div>
          </div>
        </div>
        <Footer />
