...
class MonthlyRevenueReport extends Component {
  ...
  handleSubmit = (event) => {
    ...
    axios.post('https://csit-314-cinema-booking-system.vercel.app/genMonthlyRevenueReport/', { sdate: sdate, edate: edate })
      .then(res => {
    ...
    return (
      <div className="monthlyRevenueReport">
        ...
        <form onSubmit={this.handleSubmit}>
          <label>
            Choose the month:
            <input type="number" min="1" max="12" name="month" value={month} onChange={this.handleInputChange} required />
          </label>
          <label>
            Choose the year:
            <input type="number" min="2000" max="2099" name="year" value={year} onChange={this.handleInputChange} required />
          </label>
