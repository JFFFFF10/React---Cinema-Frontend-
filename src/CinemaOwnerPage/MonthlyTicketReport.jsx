...
class MonthlyTicketReport extends Component {
  ...
  handleSubmit = (event) => {
    ...
    axios.post('https://csit-314-cinema-booking-system.vercel.app/genMonthlyTicketReport/', { sdate: startDate, edate: endDate })
      .then(res => {
    ...
    return (
      <div className="monthlyTicketReport">
        ...
        <form onSubmit={this.handleSubmit}>
            <label>
                Choose the month:
                <input type="number" min="1" max="12" step="1" name="month" value={month} onChange={this.handleInputChange} required />
            </label>
            <label>
                Choose the year:
                <input type="number" min="2000" max="2023" step="1" name="year" value={year} onChange={this.handleInputChange} required />
            </label>
